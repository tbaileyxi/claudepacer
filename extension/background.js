...
async function pollBridge() {
  try {
    ...
    // Bridge not running — that's fine, do nothing
  }
}
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg.type === "RECONNECT") {
    injectIntoOpenTabs().then(() => sendResponse({ ok: true }));
    return true;
  }
...
async function getStats() {
  ...
  return {
    ...
    // Falls back to session-persisted value if in-memory _interceptorAt was reset by SW restart.
    interceptorActive: (() => {
      const ts = _interceptorAt || 0;
      return ts > 0 && (now - ts < 60_000);
    })(),
    interceptorAt: _interceptorAt,
...
async function calibrateFromUsagePage({ weeklyPctUsed, sessionPctUsed, sessionResetAt, resetDay, resetTime }) {
  ...
  if (tokensUsed > 500 && !data.limitSetByUser) {
    ...
    console.log("[ClaudePacer] 🎯 Auto-calibrated limit:", data.weeklyLimit,
                "(", tokensUsed, "tokens =", Math.round(weeklyPctUsed * 100) + "% )");
  }
