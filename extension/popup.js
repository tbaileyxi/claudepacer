...
function init() {
  ...
  document.getElementById("btn-memory").addEventListener("click", async () => {
    ...
    document.getElementById("mem-saving").classList.remove("hidden");
    // Simulate compression (real implementation would call a backend)
    await new Promise(r => setTimeout(r, 1800));
    document.getElementById("mem-saving").classList.add("hidden");
    // Mock result — real data comes from backend after compression
    showSaveResult({ tokensBefore: 32400, tokensAfter: 18600, tokensSaved: 13800, pctSaved: 43 });
    toast("🧠 Memory file updated — 43% smaller!");
  });

  document.getElementById("btn-fresh").addEventListener("click", () => {
    if (document.getElementById("btn-fresh").dataset.locked === "true") {
      document.getElementById("btn-upgrade").scrollIntoView({ behavior: "smooth" });
      return;
    }
    toast("✨ Saving thread & opening fresh chat…");
