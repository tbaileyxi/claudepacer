...
function _cpHandleInterceptorMsg(event) {
  ...
  // message_limit — user hit a usage limit; forward to background for UI state.
  if (event.data.type === "MESSAGE_LIMIT") {
    chrome.runtime.sendMessage({
      type: "MESSAGE_LIMIT",
      resetsAt: event.data.resetsAt || null,
      remaining: event.data.remaining ?? null,
    }).catch(() => {});
    return;
  }
...
function scrapeUsagePage() {
  ...
  // Detect plan tier from the usage page text.
  // Heuristics: look around the Weekly limits section for "Pro" / "Max 5×" / "Max 20×".
  const planSlice = text.slice(Math.max(0, weeklyIdx - 500), weeklyIdx + 1200);
  let plan = null; // "pro" | "max_5x" | "max_20x"
  if (/\bMax\b/i.test(planSlice)) {
    if (/(20\s*[x×])|(Max\s*20)/i.test(planSlice)) plan = "max_20x";
    else if (/(5\s*[x×])|(Max\s*5)/i.test(planSlice)) plan = "max_5x";
    else plan = "max"; // unknown multiplier
  } else if (/\bPro\b/i.test(planSlice)) {
    plan = "pro";
  }
  ...
  // Debug removed: usage page data

  const payload = { type: "USAGE_PAGE_DATA", weeklyPctUsed, sessionPctUsed, sessionResetAt, resetDay, resetTime, plan };
