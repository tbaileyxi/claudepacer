...
(function () {
  ...
  // Debug removed: interceptor active
    ...
    try {
      while (true) {
        ...
        for (const line of lines) {
          ...
          // message_limit — user hit a plan/rate limit (often includes resetsAt / remaining).
          // This is the best signal for "out of tokens" UX in the panel.
          if (obj.type === "message_limit") {
            const resetsAt = obj?.resetsAt || obj?.resets_at || obj?.message_limit?.resetsAt || null;
            const remaining = obj?.remaining ?? obj?.message_limit?.remaining ?? null;
            window.postMessage({
  ...
  // Debug removed: stream completion
  ...
  // Debug removed: exact usage from JSON
        relay(
  ...
  // Debug removed: DOM fallback triggered
