...
A Chrome extension that gives Codex Pro/Max users a **speedometer** (token burn rate) and **gas gauge** (weekly budget) so they can see how fast they're burning through their plan limits — in real time, right beside Codex.ai.

**Tagline:** "Stop driving Codex without a speedometer or gas gauge."
...
### ✅ Working
- Extension loads and shows Live green dot after first message on Codex.ai
- Side panel opens beside Codex (persistent, not a popup)
- Speedometer + gas gauge SVGs update with estimated token counts
- Background service worker auto-injects interceptor into open Codex.ai tabs on startup
- Codex bridge (`Codex-bridge/bridge.js`) watches `~/.Codex/` and feeds desktop usage into the same gauges

### ⚠️ Limitations right now
- Token counts are **estimated**, not exact
  - Codex.ai's SSE strips `usage` fields that the standard Anthropic API includes
...
Codex Pro/Max users can't reroute through a proxy. They use Codex.ai directly.
...
Codex.ai's CSP blocks `chrome-extension://` URLs in `<script>` tags.
...
├── content.js          Runs on Codex.ai. Listens for postMessage from interceptor,
...
├── sidepanel.html      Main product UI (persistent panel beside Codex)
...
Codex-bridge/
├── bridge.js           Node.js. Watches ~/.Codex/projects/**/*.jsonl.
...
2. The background service worker will auto-inject into any open Codex.ai tabs
3. No need to reload the Codex.ai tab (unless testing content.js changes)

## How to start the Codex bridge

```bash
cd ~/Documents/ClaudePacer/Codex-bridge
/usr/local/bin/node bridge.js
```
Or double-click `start-bridge.command` in Finder.
Extension polls it every minute. Shows "⌨️ Codex" chip in side panel when active.

---

## Token counting — the core technical challenge

Codex.ai's internal API is at `/api/organizations/{orgId}/chat_conversations/{convId}/completion`.
...
Memory tools work with Codex's MEMORY.md convention:
